 ...
 */
package io.springbok.statefun.examples.greeter;

import io.springbok.statefun.examples.greeter.generated.GreetRequest;
import io.springbok.statefun.examples.greeter.generated.GreetResponse;
...
final class GreetStatefulFunction implements StatefulFunction {
  ...
  private final PersistedValue<LocalDateTime> lastSeen =
      PersistedValue.of("last-seen", LocalDateTime.class);
  ...
  private GreetResponse computePersonalizedGreeting(GreetRequest greetMessage) {
    ...
    final LocalDateTime seenTime = lastSeen.getOrDefault(currentTime);

    if (currentTime.isAfter(seenTime.plusSeconds(5))) {
      seenCount.set(0);
    }
