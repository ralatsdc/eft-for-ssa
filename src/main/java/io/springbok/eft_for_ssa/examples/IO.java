...
public class IO {
	public static final IngressIdentifier<String> INGRESS_ID = new IngressIdentifier<>(String.class, "eft", "string-in");
	public static final IngressIdentifier<Tracklet> TRACKLET_INGRESS_ID = new IngressIdentifier<>(Tracklet.class, "eft", "string-in");
	...
	public IngressSpec<String> getIngressSpec() {
		return KafkaIngressBuilder.forIdentifier(INGRESS_ID).withKafkaAddress(this.kafkaAddress).withTopic("tracklets").withDeserializer(IO.KafkaDeserializer.class).withProperty("group.id", "eft-for-ssa").build();
	}
