...
public class OrbitStatefulFunction implements StatefulFunction {
	...
	@Persisted
	private final PersistedValue<KeyedOrbit> orbitState = PersistedValue.of("orbit", KeyedOrbit.class);
	...
	public void invoke(Context context, Object input) {
		...
		if (input instanceof DelayedDeleteMessage){
			...
			context.send(OrbitIdStatefulFunction.TYPE, "manager", new RemoveOrbitMessage(orbit.getId()));

			// Send message to tracklet(s)
			ids.forEach(id ->{
				context.send(TrackletStatefulFunction.TYPE, String.valueOf(id), new RemoveOrbitMessage(orbit.getId()));
			});
		...
		if (input instanceof CollectedTrackletsMessage) {
			...
			context.send(OrbitStatefulFunction.TYPE, String.valueOf(newOrbit.getId()), new NewOrbitMessage(newOrbit));
		}
	}
