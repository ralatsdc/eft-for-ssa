...
public class TLENumericalPropagator {
  ...
  static Vector3D getPositionFromAzEl(
      AngularAzEl azel, Range range, Frame staF, Frame inertialFrame, AbsoluteDate date) {
  ...
  public static void main(final String[] args) throws IOException {
    try {
      ...
      try {
        ...
        final Orbit initialKeplerianOrbit =
            new KeplerianOrbit(
        ...
        final CorrelatedRandomVectorGenerator angularNoiseGenerator =
            new CorrelatedRandomVectorGenerator(angularCovarianceMatrix, small, gaussianGenerator);
        final double[] azElSigmas = new double[] {1, 1};
        final double[] azElBaseWeights = new double[] {1, 1};
        final AngularAzElBuilder azElBuilder =
            new AngularAzElBuilder(
                angularNoiseGenerator, groundStation, azElSigmas, azElBaseWeights, satelliteIndex);
        ...
        final CorrelatedRandomVectorGenerator rangeNoiseGenerator =
            new CorrelatedRandomVectorGenerator(rangeCovarianceMatrix, small, gaussianGenerator);
        final double sigma = 1.;
        final double baseWeight = 1.;
        final RangeBuilder rangeBuilder =
            new RangeBuilder(
                rangeNoiseGenerator, groundStation, false, sigma, baseWeight, satelliteIndex);
        ...
        final int nRev = 0;
        final KeplerianOrbit orbitEstimation =
            lambert.estimate(
                inertialFrame,
                posigrade,
                nRev,
